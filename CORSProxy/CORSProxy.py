...
class Proxy:
    """A (relatively) sinple class to perform HTTP request proxying and add COR
    ...
    def __init__(self, server_name, server_port=0, target_protocol="",
        ...
        allow_from: Controls the headers that are added to the response.  If
            ...
            from any domain.  If a list, headers will be added to allow from
            the named domains only. Entries must be in the form of a URI,
            including protocol, e.g.: "http://my-domain/".  Entries are case
            sensitive.  Note that non-CORS-requests (those that do not include
            an Origin header) will not have headers added if using a restricted
            list. However, this WILL NOT stop the request - if you wish to do
            so, use an auth function (below).
            Default: False.
        ...
        Please note: The W3C includes the following statement in their COR
            ...
            constrained. Rather than allowing a space-separated list of
            origins, it is either a single origin or the string "null".".  In
            order to account for this, this proxy will inspect the origin of
            the request and if it is in allow_from or allow_from is True or
            "*", the origin as listed in the request will be the origin listed
            in the request.  "*" may still be used if the request does not
            include an origin header.  This is most commonly the case if not
            using a CORS-compliant browser or if the request does not come via
            AJAX.
        ...
        Please note: The W3C remarks that no authentication data should be sent
            ...
            this means you may not receive standard authentication headers
            under certain circumstances (specific browsers; javascript
            libraries) if using allow_from="*".
    ...
    def proxy_start_response(self, status, headers):
        ...
        # TODO: Is this needed in general?  Should it be run by default with
        # the exception of servers known to work?
